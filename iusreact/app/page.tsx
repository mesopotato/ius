...
import { MessageCircle, User, Settings, HelpCircle, Folder, Send, Plus, Info, MessageSquare } from "lucide-react"
...
export default function Component() {
  ...
  const [messages, setMessages] = useState<{ sender: string; text: string }[]>([]); // Type annotation added
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages(prevMessages => [...prevMessages, { sender: 'bot', text: data.llm_response }]);
    ...
    <div className="flex h-screen bg-teal-600 p-4">
      ...
      <div className="w-64 bg-teal-700 p-4 text-white rounded-xl">
        ...
        <nav className="space-y-4">
          <Button variant="ghost" className="w-full justify-start text-white hover:bg-teal-600 hover:text-white">
      ...
      <div className="flex-1 flex flex-col bg-sky-100 rounded-xl ml-4">
        <header className="bg-sky-100 p-4 rounded-t-xl">
          <h2 className="text-xl font-semibold text-teal-800">Ask IUS based on Fedlex and Belex Data</h2>
        </header>
        <div className="flex-1 flex p-4">
          ...
          <div className="flex-1 flex flex-col h-[calc(100vh-150px)] bg-sky-100 rounded-xl shadow-lg mr-4 p-4 border border-teal-600">
              ...
              <div className="flex-1 overflow-y-auto border-0 bg-sky-100">
                <div className="flex flex-col space-y-4">
                  {messages.map((message, index) => (
                    <div
                      key={index}
                      className={`flex ${message.sender === 'user' ? 'justify-end pr-4' : 'justify-start pl-4'}`}
                    >
                      <div ref={messagesEndRef}
                        className={`p-2 rounded-lg max-w-[90%] ${
                          message.sender === 'user' ? 'bg-teal-600 text-white' : 'bg-sky-200 text-black'
                        }`}
            ...
            <div className="flex-shrink-0 mt-4">
              <div className="flex items-center">
                ...
                />
                <Button className="bg-teal-600 hover:bg-teal-700 text-white" onClick={handleSend}>
          ...
          {/* History Panel */}
          <div className="w-80 flex flex-col h-[calc(100vh-150px)] bg-sky-100 rounded-xl shadow-lg border border-teal-600 overflow-hidden">
              ...
              <div className="flex-1 overflow-hidden">
                <TabsContent value="history" className="h-full overflow-y-auto p-4">
                  <div className="space-y-4 mt-4">
                    {["Can I get a Pitbull in Martigny? user history table required / auth etc", "Are there any cantons in Switzerland, where Pitbulls are not restricted? user history table required / auth etc", "Are there any upcoming changes to breed restriction in Switzerland? user history table required / auth etc"].map((question, index) => (
                      <Card key={index} className="p-4 bg-sky-100 shadow-md">
                        <p className="text-sm text-teal-800">{question}</p>
                        <div className="flex justify-end mt-2">
                          <Button variant="ghost" className="text-xs mr-2 text-teal-600 hover:bg-teal-600 hover:text-white">Show</Button>
                          <Button variant="ghost" className="text-xs text-teal-600 hover:bg-teal-600 hover:text-white">Delete</Button>
                        </div>
                      </Card>
                    ))}
                  </div>
                ...
                <TabsContent value="law" className="h-full overflow-y-auto p-4">
                  <div className="space-y-4 mt-4">
                    {articles.sort((a, b) => b.similarity - a.similarity).map((article, index) => (
                      <Card key={index} className="p-4 bg-sky-100 shadow-md">
                        ...
                        <h2 className="text-md font-semibold text-sky-500">
                          <a href={article.sourcelink} target="_blank" rel="noopener noreferrer">
                            {article.source_table === "articles_bern" ? "Belex" : "Fedlex"} SRN: {article.srn}
                          </a>
                        </h2>
                        {article.book_name && <p><strong>{article.book_name}</strong></p>}
                        {article.part_name && <p><strong>{article.part_name}</strong></p>}
                        {article.title_name && <p><strong>{article.title_name}</strong></p>}
                        {article.sub_title_name && <p><strong>{article.sub_title_name}</strong></p>}
                        {article.chapter_name && <p><strong>{article.chapter_name}</strong></p>}
                        {article.sub_chapter_name && <p><strong>{article.sub_chapter_name}</strong></p>}
                        {article.section_name && <p><strong>{article.section_name}</strong></p>}
                        {article.sub_section_name && <p><strong>{article.sub_section_name}</strong></p>}
                        ...
                        <Accordion type="single" collapsible className="mt-2">
                          <AccordionItem value={`item-${index}`}>
                            ...
                            <AccordionContent>
                              <p className="text-sm text-teal-800 whitespace-pre-wrap">{processArticleText(article.full_article)}</p>
                            </AccordionContent>
                          </AccordionItem>
                        </Accordion>
                        ...
                        {article.sourcelink && (
                          <div className="mt-2">
                            <a href={article.sourcelink} target="_blank" rel="noopener noreferrer" className="text-teal-600 underline">
                              View Source
                            </a>
                          </div>
                        )}
                ...
                <TabsContent value="judgements" className="h-full overflow-y-auto p-2">
                  <div className="space-y-4 mt-4"> 
                    {documents.sort((a, b) => Number(b.similarity) - Number(a.similarity)).map((document, index) => (
